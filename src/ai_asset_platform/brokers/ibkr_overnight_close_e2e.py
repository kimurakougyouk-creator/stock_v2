...
This path is position-reducing only. It first reconciles broker-confirmed
executions into the durable local ledger, requires broker/local SPY quantity to
match at exactly one share, obtains a broker what-if SELL preview, and then may
submit at most one LIMIT/DAY Paper SELL when a dedicated opt-in is present.
Live Trading is prohibited and timeout/uncertain results are never retried.
"""
...
def run_spy_overnight_paper_close(
    *,
    limit_price: float,
    ...
    # Recover only broker-confirmed executions; this step cannot send an order.
    ...
    try:
        ...
        if confirmed is not None:
            ...
            record_confirmed_fill(
                ticker="SPY", side="SELL", filled_quantity=quantity,
                avg_fill_price=price, currency="USD", order_intent_id=intent_id,
            ...
            persisted = True
        return OvernightPaperCloseResult(
            True,
            "Paper position-reducing SELL attempted once; broker result observed only",
            intent_id, result, persisted,
        )
    finally:
        broker.disconnect()
