...
Uses exact ContractDetails plus a What-If against that same already-resolved
contract. It verifies the broker still resolves the pinned contract, reports
MKT support, provides liquid-hours metadata, and accepts a one-contract
What-If preview. No real Paper or Live order is submitted.
"""
...
from ai_asset_platform.brokers.ibkr_option_whatif import run_option_whatif_for_candidate
...
class OptionPermissionPreflightResult:
    ...
    whatif_preview_received: bool = False
    whatif_local_symbol: str | None = None
    whatif_con_id: int | None = None
...
def run_option_permission_preflight() -> OptionPermissionPreflightResult:
    if not SETTINGS.enable_ibkr_paper:
        return OptionPermissionPreflightResult(False, False, False, False, False, None, None, None, False, None, errors=("IBKR Paper is not explicitly enabled",))
    if SETTINGS.enable_live_trading or SETTINGS.live_trading_unlocked:
        return OptionPermissionPreflightResult(False, False, False, False, False, None, None, None, False, None, errors=("Live Trading safety lock is not intact",))
    ...
    if exact and port is not None:
        preview = run_option_whatif_for_candidate(
            int(port),
            candidate,
    ...
    preview_errors = tuple(getattr(preview, "errors", ()) or ()) if preview is not None else ()
    errors = tuple(discovery_errors) + preview_errors
    whatif_exact = bool(
        preview is not None
        and preview.ready
    ...
    return OptionPermissionPreflightResult(
        ...
        liquid_hours,
        whatif_exact,
        getattr(preview, "margin_change", None),
        bool(getattr(preview, "preview_received", False)),
        getattr(preview, "local_symbol", None),
        getattr(preview, "con_id", None),
        errors,
        False,
        False,
    )
...
def main() -> int:
    ...
    print("WHAT-IF PREVIEW RECEIVED    :", result.whatif_preview_received)
    print("WHAT-IF LOCAL SYMBOL        :", result.whatif_local_symbol)
    print("WHAT-IF CON ID              :", result.whatif_con_id)
