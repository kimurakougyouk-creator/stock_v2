...
def test_build_global_stock_contract_requires_explicit_fields():
    ...
    with pytest.raises(ValueError, match="symbol"):
        discovery.build_global_stock_discovery_contract(
            symbol=" ", exchange="TSEJ", currency="JPY"
        )
    with pytest.raises(ValueError, match="exchange"):
        discovery.build_global_stock_discovery_contract(
            symbol="7203", exchange=" ", currency="JPY"
        )
    with pytest.raises(ValueError, match="currency"):
        discovery.build_global_stock_discovery_contract(
            symbol="7203", exchange="TSEJ", currency=" "
        )


def test_candidate_preserves_broker_contract_metadata():
...
def test_discovery_is_contract_details_only_and_never_sends_order(monkeypatch):
    ...
    result = discovery.discover_ibkr_paper_global_stock(
        symbol="7203", exchange="TSEJ", currency="JPY", timeout=0.01
    )
