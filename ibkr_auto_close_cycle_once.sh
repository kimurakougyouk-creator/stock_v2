...
CONFIRM="${IBKR_AUTO_CLOSE_CONFIRM:-}"
...
if [[ "$CONFIRM" != "YES_CLOSE_ONE_SPY_PAPER" ]]; then
  echo "BLOCKED: set IBKR_AUTO_CLOSE_CONFIRM=YES_CLOSE_ONE_SPY_PAPER to approve one position-reducing Paper SELL. No order was sent."
  exit 2
fi
...
python -m ai_asset_platform.brokers.ibkr_auto_close_cycle
close_rc=$?
