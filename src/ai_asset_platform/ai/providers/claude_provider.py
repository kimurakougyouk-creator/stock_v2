...
ClaudeRequestFunction = Callable[
...
class ClaudeProvider:
    """
    Anthropic Claudeを利用するAIプロバイダーの基本クラス。

    現段階ではClaude APIへ直接通信せず、
    ...
    def __init__(
        ...
        *,
        ...
        request_function: ClaudeRequestFunction | None = None,
    ) -> None:
        normalized_model = str(model or "").strip()

        if not normalized_model:
            raise ValueError("Claudeのモデル名を指定してください。")
    ...
    def evaluate(
        ...
        request_functionが未設定の場合は通信せずにエラーを返す。
