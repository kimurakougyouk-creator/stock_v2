...
for ticker in tickers:
    ...
    for i in range(len(df)):
        ...
        if not position and df["Buy"].iloc[i]:
            ...
            stop_price = buy_price - df["ATR"].iloc[i] * 2.5
        ...
        # 売り
            ...
            stop_price = max(
                stop_price,
                highest_price - df["ATR"].iloc[i] * 2.5
            )
    ...
    if trade_count > 0:
        ...
        win_count = sum(1 for t in trades if t[4] > 0)
        ...
        print(f"勝率 : {win_count/trade_count*100:.2f}%")
        print(f"総利益率 : {total_profit:.2f}%")
        print(f"平均利益率 : {average_profit:.2f}%")
