...
def test_transmission_is_disabled_by_default():
    ...
    result = transmit_ibkr_paper_order(
        request, create_ibkr_paper_config(), client=client, next_order_id=100,
        guard=ready_guard(),
    )
...
def test_guard_blocks_transmission():
    ...
    guard = IbkrPaperOrderGuardResult(
        status="WAITING", allowed=False, symbol="AAPL", quantity=1, message="waiting"
    )
    result = transmit_ibkr_paper_order(
        request, create_ibkr_paper_config(), client=client, next_order_id=100,
        enable_transmission=True, guard=guard,
    )
    assert result.sent is False
    assert client.calls == []
...
def test_missing_next_order_id_blocks_transmission():
    ...
    request = OrderRequest("AAPL", OrderSide.BUY, 1)
    result = transmit_ibkr_paper_order(
        request, create_ibkr_paper_config(), client=client, next_order_id=None,
        enable_transmission=True, guard=ready_guard(),
    )
...
def test_live_configuration_is_blocked():
    ...
    config = IbkrConnectionConfig(
        host="127.0.0.1", port=7496, client_id=0,
        paper_trading=False, allow_live_trading=True,
    )
    result = transmit_ibkr_paper_order(
        request, config, client=client, next_order_id=100,
        enable_transmission=True, guard=ready_guard(),
    )
...
def test_default_guard_uses_tws_port_when_config_is_tws(monkeypatch):
    ...
    request = OrderRequest("AAPL", OrderSide.BUY, 1)
    result = transmit_ibkr_paper_order(
        request, create_ibkr_paper_config(use_gateway=False), client=client,
        next_order_id=100,
    )
...
def test_default_guard_uses_gateway_port_when_config_is_gateway(monkeypatch):
    ...
    request = OrderRequest("AAPL", OrderSide.BUY, 1)
    result = transmit_ibkr_paper_order(
        request, create_ibkr_paper_config(use_gateway=True), client=client,
        next_order_id=100,
    )
...
def test_explicit_enable_sends_one_paper_order():
    ...
    request = OrderRequest("AAPL", OrderSide.BUY, 1)
    result = transmit_ibkr_paper_order(
        request, create_ibkr_paper_config(), client=client, next_order_id=321,
        enable_transmission=True, guard=ready_guard(),
    )
...
def test_explicit_etf_instrument_reaches_contract_and_remains_unsent_by_default():
    ...
    result = transmit_ibkr_paper_order(
        request, create_ibkr_paper_config(), client=client, next_order_id=777,
        guard=ready_guard("SPY", 1), instrument=instrument,
    )
    assert result.status == "READY_NOT_SENT"
    assert result.sent is False
